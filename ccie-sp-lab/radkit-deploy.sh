...
# STEP 3 - Define expect wrapper function
run_with_password() {
    local CMD="$1"
    expect <<EOD
...
UBUNTU1_OUTPUT=$(expect <<EOD
spawn docker exec -it radkit bash -c "radkit-control device create ubuntu1 10.253.11.11 Linux --description ubuntu1 --jumphost-uuid $UBUNTU0_UUID  --terminal-connection-method SSH --terminal-username ubuntu --terminal-password cisco --forwarded-tcp-ports 22 --active true"
expect "superadmin's password:"
send "Cisco123!\r"
expect eof
EOD
...
echo "Ubuntu1 created. Output:"
echo "$UBUNTU1_OUTPUT"

# Extract UUID
UBUNTU1_UUID=$(echo "$UBUNTU1_OUTPUT" | grep -o '"uuid": "[^"]*' | awk -F'"' '{print $4}')
...
echo "Ubuntu2 created. Output:"
echo "$UBUNTU2_OUTPUT"

# Extract UUID
UBUNTU2_UUID=$(echo "$UBUNTU2_OUTPUT" | grep -o '"uuid": "[^"]*' | awk -F'"' '{print $4}')

if [[ -z "$UBUNTU2_UUID" ]]; then
    echo "Failed to capture ubuntu2 UUID. Exiting."
    exit 1
fi
